...
class PNetLin(nn.Module):
    def __init__(self, pnet_type='vgg', pnet_rand=False, pnet_tune=False, use_dropout=True, use_gpu=True, spatial=False, version='0.1'):
        ...
        print("PNetLin model instantiating")
        print("-"*40)
        self.use_gpu = use_gpu
        self.pnet_type = pnet_type
        ...
        if(self.pnet_type in ['vgg','vgg16']):
            net_type = pn.vgg16 # get vgg16 architecture class, divided into chunk of slices as named tuple
            """
            # to check architecture
            net = net_type()
            print("chuncked vgg16:", net)
            """
        ...
        if(self.pnet_tune):
            print("vgg16 will be tuned (requires_grad=True) & will be part of PNetLin model architecture")
            self.net = net_type(pretrained=not self.pnet_rand,requires_grad=True)
        else:
            print("vgg16 will not be tuned (requires_grad=False) & will not be part of PNetLin model architecture")
            # 'not self.pnet_rand' = not False = True, so we download pretrained vgg26
            self.net = [net_type(pretrained=not self.pnet_rand,requires_grad=False),]

        # print("self.net:", self.net)
        # create linear layers, mapping to one channel
        self.lin0 = NetLinLayer(self.chns[0],use_dropout=use_dropout) # (64, 1)
        self.lin1 = NetLinLayer(self.chns[1],use_dropout=use_dropout) # (128, 1)
        self.lin2 = NetLinLayer(self.chns[2],use_dropout=use_dropout) # (256, 1)
        self.lin3 = NetLinLayer(self.chns[3],use_dropout=use_dropout) # (512, 1)
        self.lin4 = NetLinLayer(self.chns[4],use_dropout=use_dropout) # (512, 1)
        # create a list of linear layers
        self.lins = [self.lin0,self.lin1,self.lin2,self.lin3,self.lin4]
        
        # if 'pnet_type=seqeeze', we need 7 layers so we add two extra
        if(self.pnet_type=='squeeze'):
            self.lin5 = NetLinLayer(self.chns[5],use_dropout=use_dropout)
            self.lin6 = NetLinLayer(self.chns[6],use_dropout=use_dropout)
            self.lins += [self.lin5,self.lin6]

        # what is this shift and scale ?
        self.shift = torch.autograd.Variable(torch.Tensor([-.030, -.088, -.188]).view(1,3,1,1))
        self.scale = torch.autograd.Variable(torch.Tensor([.458, .448, .450]).view(1,3,1,1))

        if(use_gpu):
            # if model is being tuned, put it on cuda
            if(self.pnet_tune):
                self.net.cuda()
            else:
                # put the first element of list (self.net), which is the chunked vgg16, on cuda
                self.net[0].cuda()
              
            # put shift & scale autograd variable on cuda
            self.shift = self.shift.cuda()
            self.scale = self.scale.cuda()
            # put the linear layers on cuda
            self.lin0.cuda()
            self.lin1.cuda()
            self.lin2.cuda()
            self.lin3.cuda()
            self.lin4.cuda()
            # if 'pnet_type' is 'squeeze', put the additional two layers on cuda
            if(self.pnet_type=='squeeze'):
                self.lin5.cuda()
                self.lin6.cuda()
...
"""
A single linear layer, w/wo dropout which does a 1x1 conv
"""
...
"""
Print the network architecture and number of parameters for that model
"""
