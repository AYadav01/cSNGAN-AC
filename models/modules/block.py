...
def act(act_type, inplace=True, neg_slope=0.2, n_prelu=1):
    act_type = act_type.lower() # convert lowercase
...
def get_valid_padding(kernel_size, dilation):
    # print("checking for valid padding")
    # a = kernel_size
    kernel_size = kernel_size + (kernel_size - 1) * (dilation - 1)
    # print("kernel size = {} + ({} - 1) * ({} - 1) = {}".format(a, a, dilation, kernel_size))
    padding = (kernel_size - 1) // 2
    # print("padding = ({}-1/2) = {}".format(kernel_size, padding))
    return padding
...
def sequential(*args):
    ...
    modules = []
    for module in args:
        # if 'module' is instance of 'nn.Sequential', we unpack the childrens of 'module' since it will have
        # more than one layer.
