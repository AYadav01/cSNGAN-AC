...
class DistModel(BaseModel):
    ...
    """
    Initializes pdst model (e.g. PNetLin), loads weights and sets up for either training or evaluation
    """
    def initialize(self, model='net-lin', net='alex', pnet_rand=False, pnet_tune=False, model_path=None, colorspace='Lab', use_gpu=True, printNet=True, spatial=False, spatial_shape=None, spatial_order=1, spatial_factor=None, is_train=False, lr=.0001, beta1=0.5, version='0.1'):
        """
        INPUTS
        ...
        print("dist_model initialize function called")
        ...
        print("spatial factor:", self.spatial_factor)

        # model name: net-lin [vgg]
        self.model_name = '%s [%s]'%(model, net)
        print("model name:", self.model_name)

        if(self.model == 'net-lin'): # pretrained net + linear layer

            # use_gpu=True, pnet_rand=False, pnet_tune=False, pnet_type=vgg, spatial=False, version=0.1
            ...
            if not use_gpu:
                print("map_location will be cpu")
                kw['map_location'] = 'cpu'

            # since 'model_path' is None, inspect for vgg16 weights path based on weights version (0.1)
            if(model_path is None):
                import inspect
                # model_path = './PerceptualSimilarity/weights/v%s/%s.pth'%(version,net)
                model_path = os.path.abspath(os.path.join(inspect.getfile(self.initialize), '..', '..', 'weights/v%s/%s.pth'%(version,net)))

            # 'is_train' is False; not False is True, so we load the vgg16 weights to the model
            if(not is_train):
        ...
        # get all the parameters from the network
        self.parameters = list(self.net.parameters())

        # if 'self.is_train' is True, we add extra network (self.rankLoss) on top to go from 
        # distances (d0, d1)=> predicted human judgment (h*), and then add 'self.rankLoss' model
        # parameters to existing 'self.parameters' list, and also set up optimizer
        if self.is_train: # training mode
            # extra network on top to go from distances (d0,d1) 
            self.rankLoss = networks.BCERankingLoss(use_gpu=use_gpu)
            self.parameters+=self.rankLoss.parameters
            self.lr = lr
            self.old_lr = lr
            self.optimizer_net = torch.optim.Adam(self.parameters, lr=lr, betas=(beta1, 0.999))
        else: # put model on evaluation mode
            self.net.eval()

        # print the pdist model, if specified
        if(printNet):
            print('---------- Pdist Networks initialized -------------')
